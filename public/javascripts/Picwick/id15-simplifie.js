// Lien du test
https://www.picwictoys.com/p/crayola-4-tubes-de-peinture-aux-doigts-1756095?abtestduplicate

if (!window.abt622704) {
      window.abt622704 = true;
      ...
      setTimeout(function(){
        ...
        jQuery(document).on('mousedown', '#myModal .close', function () { ABTastyClickTracking('Clic bouton fermeture pop-in', null, 622704); });
        jQuery(document).on('mousedown', '#myModal .buttonAchats', function () { ABTastyClickTracking('Clic bouton continuer achats', null, 622704); });
        jQuery(document).on('mousedown', '#myModal .buttonPanierPopin', function () { ABTastyClickTracking('Clic bouton voir panier', null, 622704); });
     ...
     jQuery('.Snackbar-bar--success').remove();
     jQuery('.Snackbar-wrap').remove();

      setInterval(function(){
        if (stopintervalab === true){
          jQuery('.Button:contains(Ajout au panier)').click(function(event) {
...
// SOLUTION A
 ...
 if (!window.abt632374) {
     ...
     setTimeout(function(){
       ...
       jQuery('.modal').css({'position': 'fixed', 'z-index': '9999', 'left': '0', 'top': '0', 'overflow': 'auto', 'width': '100%', 'height': '100%', 'background-color': 'rgba(53, 192, 236, 0.7)'});
       jQuery('.modal-content').css({'background-color': 'white', 'margin': '40% auto', 'padding': '20px', 'width': 'auto', 'height': '260px', 'border-radius': '10px', 'margin-left': '5px', 'margin-right': '5px'});
       jQuery('.containerSuccess').css({'display': 'flex', 'align-items': 'center', 'justify-content': 'center'});
       jQuery('.pictoSuccess').css({'margin-right': '10px'});
       jQuery('.messageSuccess').css({'font-size': '15px', 'color': 'rgba(53, 192, 236)', 'font-weight': '501'});
    ...
    jQuery('.Snackbar-wrap').remove();
     ...
     setInterval(function(){
       if (stopintervalab === true){
         jQuery('.Button:contains(Ajout au panier)').click(function(event) {
           if(jQuery('.Loader-content').length > 0) {
             console.log('bien détecté')
             jQuery('.HeaderContent').hide();
             event.stopPropagation();
             jQuery('.modal').hide();
             jQuery('.Head').css({'background-color': 'rgba(53, 192, 236, 0.7)'});
       ...
       if(jQuery(".ProductDeliveries-storeAction:contains(Changer de magasin)").length > 0){
         jQuery('.Button:contains(Retrait magasin en 1h)').click(function(event) {
           jQuery('.HeaderContent').hide();
           console.log('ok abtest');
           event.stopPropagation();
           jQuery('.modal').show();
   ...
   if (!window.abt632374) {
      ...
      jQuery('.Snackbar-wrap').remove();
       ...
       setInterval(function(){
         if (stopintervalab === true) {
          jQuery('.Button:contains(Ajout au panier)').click(function(event) {
            if(jQuery('.ProductDeliveries').has("Loader").length){
             jQuery('.HeaderContent').hide();
             event.stopPropagation();
             jQuery('.modal').hide();
             jQuery('.Head').css({'background-color': 'rgba(53, 192, 236, 0.7)'});
